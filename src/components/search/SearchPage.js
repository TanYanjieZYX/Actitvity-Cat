...
class SearchPage extends PureComponent{
  ...
  getText (disabled) {
    ...
    if(currentStart && currentEnd){
      return currentChannel.name + ' Activities from ' + currentStart.format('MM/DD') + ' to ' + currentEnd.format('MM/DD');
    }else{
  ...
  changeSearchParams = (type, value) => {
    ...
    if (type === 'time') {
      if (value === SEARCH_TIMES.ANYTIME) {
        ...
        result.start = moment().add(1, 'days').startOf('day')
        result.end = moment().add(1, 'days').endOf('day')
...
export default connect(mapStateToProps,{fetchChannels})(SearchPage);